...
const cue = `**${name}:** Use \`${config.prefix}mods dom5 [chosen server name]\` (without brackets) to receive a list of the mods available in the server you chose. Type \`none\` for no mods, or copy and paste (letter-case counts) one or more mod filenames, separated by commas (,).`;
...
module.exports.validate = function(input, validatedSettings, server, cb)
{
  ...
  modList.forEachAsync(function(mod, index, next)
  {
    ...
    server.socket.emit("validateMod", {mod: mod, gameType: config.dom5GameTypeName}, function(err)
    {
      if (err)
      {
        cb(err);
