...
{
  return `Allows you to upload a map or a mod to the server through google drive. To use it, you must specify whether it's a dominions 4 or 5 mod or map, and then add the google drive file sharing link at the end of the command (which can be found when you right click a file on the drive website and click on Get Shareable Link). The file must be a .zip file containing the mod or map as is meant to be extracted into the mods or maps folder. If files with the same name exist in the server, they will be overwritten if they are mod image files or map files, but .dm files will *not* be overwritten so as to not have an impact on ongoing games using that mod. To upload an updated version of a mod, just make sure you change the name to add the version number to it so it does not conflict. Please keep in mind that for bandwith reasons, each user can only upload ${userUploadLimitPerDay} map and ${userUploadLimitPerDay} mod zipfiles every day. You can upload more than one map and mod in the same zipfile, just make sure they are all zipped in the same structure in which it'll be extracted to the maps or mods folder.`;
};
