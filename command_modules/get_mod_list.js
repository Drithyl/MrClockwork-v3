...
const mapsRegexp = new RegExp(`^${config.prefix}MODS`, `i`);
...
{
  return `Receive a list of the dom4/5 mods available in the given server. You can see a list of servers if you use this command without any arguments.`;
};
...
module.exports.invoke = function(message, command, options)
{
  ...
  {
    message.channel.send(`Here is a list of the servers currently online. You must specify for which server you want to see the mods by adding its name at the end of the command, i.e. \`${config.prefix}maps dom5 myChosenServer\`:\n\n\`\`\`${slaveServersModule.getList()}\`\`\``);
    return;
  }

  if (typeof options.args[0] !== "string" || (dom4Regexp.test(options.args[0]) === false && dom5Regexp.test(options.args[0]) === false))
  {
    message.channel.send(`You must specify which server's mods you want to see; \`${config.dom4GameTypeName}\` or \`${config.dom4GameTypeName}\`.`);
    return;
  }

  hoster.sendModList(options.args[0], options.args[1], message);
};
